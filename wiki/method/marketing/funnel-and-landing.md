...
## Traffic intent routing — split bằng ai đang đến, không phải họ gõ gì ([[ecom-amin-pdp-traffic-intent-split]]) ⭐

eCom_Amin: 2 người cùng search "nike air force 1s" — 1 sẵn checkout trong 30 giây, 1 còn đang cân nhắc. Cùng 1 query, 2 buyer khác hẳn, 1 PDP chỉ serve được 1 trong 2. Fix: **route theo intent người đến, không phải theo keyword họ gõ**.

| Traffic type | Trang đích | Ghi chú |
|---|---|---|
| **Branded search** | PDP + light sales copy | Skeptic cần nudge nhẹ; KHÔNG chôn buy-button dưới 2,000 chữ — ready buyer bounce sang đối thủ ngay |
| **Non-branded search** | Advertorial hoặc listicle | 2 rule counter-intuitive: (1) **nói thẳng sản phẩm KHÔNG hợp ai** — trang nói hợp mọi người resonates with no one; trang nói "skip this if you're X" earns trust với tất cả người còn lại. (2) **full problem stack trước khi lộ giá** — sleep → cortisol → testosterone → libido; bỏ qua chain này, giá ra bị "meh, tôi chịu đựng được" |
| **Non-branded + product-aware** | Quiz funnel | Quiz cá nhân hoá thay vì giải thích — rules: branching thật (đáp án thay đổi kết quả), câu hỏi nâng cao không làm drop-off, dopamine mỗi click, ngôn ngữ người thật. **Thu email + SMS TRƯỚC khi show result** — người hoàn thành quiz đã prove intent; list đó in tiền về sau |
| **Shopping traffic** | PDP + sales angle | VSL nhúng hoặc quiz variant nếu sản phẩm cần giải thích |
| **Demand gen** | Full advertorial | Traffic lạnh nhất trên Google — ad bán click, page bán sản phẩm; KHÔNG skip thẳng vào pitch |
| **High-sophistication ("X vs Y")** | Comparison page | Unbiased framing, criteria mình chọn, honest losses included |

**Nguyên lý:** match page với temperature → cùng ad spend nhân lên. Tie [[market-awareness]] (awareness stage → format) · [[presell-pages]] §4-format (build chi tiết từng loại) · [[quiz-funnels]] §congruency (product-aware non-branded = đúng điều kiện quiz ăn nhất).

> [!tip] MaxPro
> Branded "maxpro nail grinder" → PDP luôn. Non-branded "dog nail grinder" → advertorial/listicle (full problem stack: anxiety → quick-crush → splinter → vet bill). Product-aware "maxpro vs casfuy" → [[presell-pages]] §comparison. Shopping → PDP + sales angle. Demand gen Pinterest/YouTube → full advertorial.
...
> [!note] Gruns teardown 2026 ([[rubiinov-gruns-100m-funnel-teardown]]) — angle-matched listicles + 5-part stack
> $0→$100M/2 năm (AG1 mất 10 năm cùng số). Không phải 1 tactic — **combination 5 thứ chạy đồng thời** (đa số brand chỉ 1-2/5 → đó là gap):
> 1. **Creative velocity 900+ ad** active (điển hình $30-80k/mo brand chỉ 5-20), mix UGC + static + AI → [[creative-testing]].
> 2. **Angle-matched listicles:** 1 primary LP + series listicle **per angle** (nutrition "7 benefits users love" · weight-loss · kids · protein · fiber), mỗi ad→đúng LP-ngôn-ngữ. = §landing-page-stack + message-match ở trên, xác nhận thêm bằng case $100M.
> 3. **Quiz-style pop-up 15-20% opt-in** → [[quiz-funnels]].
> 4. **Email entertain-first 3-4x/tuần** → [[email-marketing]].
> 5. **SMS immediate-conversion** → [[sms-flows]].
> *"Fix the post-click experience before you scale ad spend"* — scale spend vào leaky funnel chỉ mất tiền nhanh hơn.
...
**Stack thay thế ngoài Shopify cho pure-DR ([[the-new-direct-response-meta]]):** 1 operator DR mô tả kiến trúc tách hẳn frontend/backend — frontend build trên **Lovable** (AI no-code), backend cố định trên **Checkout Champ** (checkout + post-purchase flow + order CMS), không theme Shopify, không app-stack, không phụ thuộc dev.
- **Nguyên tắc cốt lõi: frontend DÙNG-XONG-BỎ, backend BẤT BIẾN.** Khi backend (checkout + upsell) đã convert tốt → không đụng vào nữa; mọi thử nghiệm angle mới chỉ là spin-up 1 frontend page MỚI trỏ về CÙNG backend. Đảo ngược thói quen phổ biến (dựng lại NGUYÊN funnel — page mới + checkout mới + upsell mới — mỗi lần test 1 angle) → rút thời gian build từ 2-3 ngày xuống vài giờ, test được NHIỀU angle song song thay vì lần lượt.
- **Cách build nhanh:** chụp full-page screenshot template thích (GoFullPage) + kéo element rời từ trang khác đã lưu → nạp hết vào Lovable cùng base-template → gần như "one-shot" ra trang mới; sau khi có 1 template chạy được, các lần sau chỉ đổi copy.
- **Pipeline nghiên cứu copy đi kèm (tách biệt, dùng được với BẤT KỲ stack nào):** VOC thủ công trước (Facebook group / Reddit / Amazon review — chữ THẬT khách dùng, không phải chữ mình đoán) → dựng avatar-sheet từ đó → bổ sung nghiên cứu AI (Gemini Deep Research cho market context rộng, Perplexity cho mechanism/ingredient/study) → gom hết thành tài liệu (psychographics, ingredient-copy, mechanism-angle, awareness-level) → nạp vào 1 Claude project để AI hiểu đủ ngữ cảnh trước khi viết. Research sâu = copy convert; research rác = copy rác — bottleneck luôn là research, không phải build page.
- **Đối trọng — khi nào VẪN nên giữ Shopify:** tác giả gốc tự nhận vẫn dùng Shopify cho CMS đơn hàng tiện + để xây "brand" (Shopify page hợp cho brand-building hơn stack DR thuần). Stack Lovable+CheckoutChamp hợp nhất khi mục tiêu là DR thuần tuý (presell → checkout → upsell in), chưa cần xây thương hiệu dài hạn.
