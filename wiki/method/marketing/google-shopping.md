...
> [!tip] Xác nhận độc lập: keyword-first title doubled CVR + ROAS ([[ecomamin-shopping-title-keyword-first]], [[ecom-amin-shopping-title-cvr-double-2026]])
> @eCom_Amin split test live: **cùng product, cùng bids, cùng budget** — đổi brand-first → keyword-first → **CVR doubled, ROAS nearly doubled**. Sai: *"BrandName Premium Collagen Peptides 500mg"*. Đúng: *"Collagen Supplement for Joint Pain + Skin | BrandName"*. **Cascade effect (2026 xác nhận):** CVR tăng → Google thưởng better placement → nhiều impression hơn at lower CPCs → ROAS tốt hơn → **organic rank cũng tăng từ paid Shopping performance** (Google rewards converting products across both paid and organic — không phải SEO, từ paid Shopping conversion). **Meta leak mechanism (cực quan trọng):** Meta ad tạo category interest → buyer mở Google gõ category query → listing brand-first không match → *competitor keyword-first listing thắng click*. **"You paid the CPM on Meta and they captured the conversion on Google — the only barrier was the first three words of a product title."** → 70-80% Google ecom revenue chạy qua Shopping; titles sai = mọi campaign fight với 1 tay sau lưng. Fix: vào Merchant Center, pull top-10 sản phẩm theo revenue, check 3 từ đầu — nếu brand-first, đang mất tiền ngay hôm nay.
